...
// Global Variables
//int Verbose=FALSE;
BOOL bflagall=TRUE;    // This is TRUE if the /all option is specified at the command prompt  
...
static const TCHAR* getNodeTypeStr(UINT uiNodeType){
	TCHAR* BROADCAST_NODETYPE_STR = L"Broadcast node";
	TCHAR* PEER_TO_PEER_NODETYPE_STR =L"Peer to Peer node";
	TCHAR* MIXED_NODETYPE_STR = L"mixed node";
	TCHAR* HYBRID_NODETYPE_STR = L"hybrid node";
	switch (uiNodeType){
		case BROADCAST_NODETYPE:
			return BROADCAST_NODETYPE_STR;
		case PEER_TO_PEER_NODETYPE:
			return PEER_TO_PEER_NODETYPE_STR;
		case MIXED_NODETYPE:
			return MIXED_NODETYPE_STR;
		case HYBRID_NODETYPE:
			return HYBRID_NODETYPE_STR;
		default:
...
int GetConfigData()
{
	...
	if (bflagall == TRUE) {
		OutputMessage(TEXT("\t NODETYPE........... :  %s\r\n"), getNodeTypeStr(pNetworkParams->NodeType)); //No details in the documentation regarding the correspondence between integer constants and nodetypes   
